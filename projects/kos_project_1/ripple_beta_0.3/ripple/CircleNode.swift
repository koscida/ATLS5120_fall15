...
class CircleNode: SKNode {
    ...
    init(frameWidth: CGFloat, frameHeight: CGFloat, good: Bool, color: SKColor, maxRadius: CGFloat) {
        super.init()
        ...
        self.position = CGPoint(
            x: CGFloat(arc4random_uniform( UInt32((maxX-maxRadius) + (maxRadius/2)) )),
            y: CGFloat(arc4random_uniform( UInt32((maxY-maxRadius) + (maxRadius/2)) ))
        )
    ...
    func update(radiusUpdate: CGFloat) {
        radius += radiusUpdate
        cirAlpha -= (radiusUpdate/_maxRadius)
    }
